...
SET @grant_query = CONCAT ('GRANT ALL PRIVILEGES ON ', @iiq_tag, '.* TO "', @iiq_user, '"@"', @host_value,'"');
SET @grant_plugin_query = CONCAT ('GRANT ALL PRIVILEGES ON ', @iiq_tag, 'plugin.* TO "', @iiq_user, '"@"', @host_value,'"');
