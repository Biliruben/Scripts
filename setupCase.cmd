@rem duplication of 'setupCase.cmd'.  TODO: abstract the meat into a common script
@rem and extract case v. case differences into setupCase/Case
@set doEcho=off
@echo %doEcho%
set ERROR_MSG=
if "%~1"=="" set ERROR_MSG=Two parameters must be passed: The IIQ Version and the Case Number
if "%~2"=="" set ERROR_MSG=Two parameters must be passed: The IIQ Version and the Case Number
...
call %scriptsdir%\setupCaseEnv.cmd %*

rem A little opp for our caller to complain
cd /d %REPO_HOME%
echo Finding previous instances of case: %IIQ_BUG%
...
rem - convert DDL to use tag 'iiq[version]_c[caseNumber]'
popd
popd
