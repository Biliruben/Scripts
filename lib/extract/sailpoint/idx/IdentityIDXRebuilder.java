...
import sailpoint.idx.IdentityIDXRebuilder;
...
/*
 * TODO: Task execution should be handled by the IDXSQLAnalyzerExecutor, so remove the AbstractTaskExecutor
 */
public class IdentityIDXRebuilder extends AbstractTaskExecutor implements HibernateIndexRebuilder {
	...
	public void reorderList(SailPointContext context, String id,
		...
		if (identity == null) {
			...
			errHandler.addMessage(new Message(Type.Warn, "Identity null: " + id, (Object[])null));
			return;
		}
			...
			for (Link link : allLinks) {
				...
				if (link != null) {
					...
					if (parent != null) {
						...
						errHandler.addMessage(new Message(Type.Warn, link.getId() + " is an orphan.", (Object[])null));
					}
				}
