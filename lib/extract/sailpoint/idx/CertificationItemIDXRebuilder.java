...
import sailpoint.idx.CertificationItemIDXRebuilder;
	...
	public void reorderList(SailPointContext context, String certificationEntityId, MessageAccumulator handler) {
		...
		try {
			...
			*/
			...
			for (CertificationItem item : allItems) {
				...
				if (item != null) {
					...
					if (parent != null) {
						...
						handler.addMessage(new Message(Type.Warn, item.getId() + " is an orphan.", (Object)null));
					}
				}
			...
			handler.addMessage(new Message(Type.Error, e.getMessage(), (Object)null));
		}
